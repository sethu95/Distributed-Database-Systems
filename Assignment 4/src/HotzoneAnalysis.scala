...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val joinDf = spark.sql("select rectangle._c0 as rectangle, point._c5 as point from rectangle,point where ST_Contains(rectangle._c0,point._c5)").groupBy("rectangle").count().orderBy("rectangle")

    joinDf.createOrReplaceTempView("joinResult")
    return joinDf.coalesce(1)
  }
